...
class RequestControllerTest {
    ...
    @Test
    void malformedJsonReturnsStructuredErrorWithMatchingRequestId() throws Exception {
        var result = mockMvc.perform(post("/v1/requests").contentType(MediaType.APPLICATION_JSON).content("{bad"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error.code").value("validation_failed")).andReturn();
        var body = new com.fasterxml.jackson.databind.ObjectMapper().readTree(result.getResponse().getContentAsString());
        org.junit.jupiter.api.Assertions.assertEquals(result.getResponse().getHeader("X-Request-Id"),
                body.get("error").get("request_id").asText());
    }

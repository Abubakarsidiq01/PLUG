...
public class SecurityConfiguration {
        ...
        return http.csrf(csrf -> csrf.ignoringRequestMatchers("/v1/requests"))
                ...
                .exceptionHandling(errors -> errors
                        .authenticationEntryPoint((request, response, exception) ->
                                HttpErrors.write(request, response, 401, "unauthorized", "Authentication is required."))
    ...
    SecurityFilterChain local(HttpSecurity http) throws Exception {
        return base(http).authorizeHttpRequests(auth -> auth
                .requestMatchers(HttpMethod.GET, "/health", "/actuator/health", "/actuator/health/readiness", "/actuator/health/liveness").permitAll()
    ...
    SecurityFilterChain staging(HttpSecurity http) throws Exception {
        return base(http).authorizeHttpRequests(auth -> auth
                ...
                .oauth2ResourceServer(resource -> resource.jwt(jwt -> {})
                        .authenticationEntryPoint((request, response, exception) ->
                                HttpErrors.write(request, response, 401, "unauthorized", "A valid access token is required.")))
                .build();
    }

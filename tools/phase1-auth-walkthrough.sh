...
n=0
status() { n=$((n+1)); curl -s -o /tmp/body.json -w '%{http_code}' "$@"; cp /tmp/body.json /tmp/resp-$n.json; }
...
GUEST_AT=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['access_token'])")
GUEST_UID=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['account']['user_id'])")
GUEST_SCOPES=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['account']['scopes'])")
...
check "  error code" '"forbidden"' "$(python3 -c "import json;print(json.dumps(json.load(open('/tmp/body.json'))['error']['code']))")"
...
CHALLENGE=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['challenge_id'])")
CODE=$(tail -1 "$CODES" | awk '{print $NF}')
# Captured into its own file so the assertion cannot read another call's body.
WRONG=$(curl -s -o /tmp/wrong.json -w '%{http_code}' -X POST $API/v1/auth/phone/verify \
  -H 'Content-Type: application/json' \
  -d "{\"challenge_id\":\"$CHALLENGE\",\"code\":\"000000\",\"consent_version\":\"$CONSENT\"}")
check "wrong code -> validation_failed" 400 "$WRONG"
check "  details[0].code" '"invalid"' "$(python3 -c "import json;print(json.dumps(json.load(open('/tmp/wrong.json'))['error']['details'][0]['code']))")"
check "  error code" '"validation_failed"' "$(python3 -c "import json;print(json.dumps(json.load(open('/tmp/wrong.json'))['error']['code']))")"
...
MEM_AT=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['access_token'])")
MEM_RT=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['refresh_token'])")
MEM_UID=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['account']['user_id'])")
check "user_id preserved across upgrade" "$GUEST_UID" "$MEM_UID"
check "account type" '"phone"' "$(python3 -c "import json;print(json.dumps(json.load(open('/tmp/body.json'))['account']['type']))")"
...
ROT_AT=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['access_token'])")
ROT_RT=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['refresh_token'])")
...
C2=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['challenge_id'])"); K2=$(tail -1 "$CODES" | awk '{print $NF}')
...
A_AT=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['access_token'])")
status $API/v1/me/sessions -H "Authorization: Bearer $A_AT" >/dev/null
A_SID=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['sessions'][0]['session_id'])")

status -X POST $API/v1/auth/phone/start -H 'Content-Type: application/json' -d "{\"phone_number\":\"$(num 3)\"}" >/dev/null
C3=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['challenge_id'])"); K3=$(tail -1 "$CODES" | awk '{print $NF}')
...
B_AT=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['access_token'])")
...
check "  field" '"consent_version"' "$(python3 -c "import json;print(json.dumps(json.load(open('/tmp/body.json'))['error']['details'][0]['field']))")"
...
C4=$(python3 -c "import json;print(json.load(open('/tmp/body.json'))['challenge_id'])")
last=""
for i in 1 2 3 4 5 6; do
  last=$(status -X POST $API/v1/auth/phone/verify -H 'Content-Type: application/json' -d "{\"challenge_id\":\"$C4\",\"code\":\"00000$i\",\"consent_version\":\"$CONSENT\"}")
done

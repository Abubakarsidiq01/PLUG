...
| P0.S3 | `/health`, `/health/ready`, error envelope, correlation filter | person_one | frozen@0.1.0 (pending joint review) | — | — | Contract test validates real responses against OpenAPI | No config leak on `/health`; readiness kept off public internet at infra layer | Verified live: `curl localhost:8080/health(/ready)`, `ContractTest` | building | `/health/ready` needs a WAF/security-group rule once staging exists |
| P0.S4 | `POST /v1/requests` validated stub | person_one | frozen@0.1.0 (pending joint review) | P0.S3 | — | 202 on valid input, structured 400 on invalid | Strict schema validation, no persistence yet | `RequestControllerTest`, Bruno `requests-create-*.bru` | building | — |
...
| P0.S7 | iOS Xcode project, APIClient, Engineering health screen | person_one | — | — | — | Runs on Simulator + a real device | Keychain untouched in P0 | Verified live: `xcodebuild test -scheme Plug` passes, app installs and launches on iPhone 17 Simulator | building | Not yet run on a physical device — manual.docx explicitly calls this out ("the Simulator hides real network behaviour") |
| P0.S8 | GitHub Actions: backend, iOS, security, OpenAPI | person_one | — | — | — | All required checks green on a PR | CodeQL + gitleaks + Dependabot | `.github/workflows/` | building | Not yet observed running in a real PR |
...
| P0.S1-two | Windows setup proof | person_two | — | — | — | Every documented command runs | — | | not started | No Windows machine has run `docs/onboarding/windows.md` yet |
| P0.S2-two | `/web` public site + protected admin shell | person_two | — | — | — | `pnpm --filter @plug/web build` succeeds; protected route redirects | Server-independent redirect now, real server-side auth in P1 | `web/`, Playwright `phase0-smoke.spec.ts` (9/9 passing, 3 viewports) | building | Admin auth is a placeholder cookie check, not real auth (by design — that's P1) |
...
| P0.S4-two | Playwright baseline | person_two | — | — | — | `pnpm --filter @plug/web test:e2e` passes on a clean clone | — | 9/9 tests passing across mobile/tablet/desktop | done | — |
| P0.S5-two | Bruno collection | person_two | — | — | — | `bru run tests/api --env staging` passes | — | Verified live against local backend: 4/4 requests, 10/10 assertions | building | Not yet run against real staging (doesn't exist) |
...
| P0.S8-two | Windows onboarding doc | person_two | — | — | — | Every step runs without an undocumented prerequisite | — | `docs/onboarding/windows.md` | not started | Unvalidated — written from the manual, not proven on real Windows hardware |
